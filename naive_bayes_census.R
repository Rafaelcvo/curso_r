...
#removendo a coluna x
base$X = NULL

#Contagem de todos os registros
table(base$sex)
table(base$relationship)

#niveis 
unique(base$sex)

#transformando os atributos em valor
...
#verificando se gerou falore NA
base[is.na(base$sex)]
...
library(e1071)
library(caret)
classif <- naiveBayes(x = x_treino[-15], y = x_treino$income)
print(classif)
previsoes <- predict(classif, x_teste[-15])
matriz_confusao <- table(x_teste[,15], previsoes)
